    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => set("prenom_marie1", e.target.value)}
              placeholder="Zineb"
              className={inputCls}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => set("prenom_marie2", e.target.value)}
              placeholder="Farès"
              className={inputCls}
            />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Paris, Salle des fêtes…"
              className={inputCls}
            />
          </div>
        </div>
      ...
      <div className="glass border border-[#E8A87C]/15 p-6 rounded-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => set("total_ttc", e.target.value)}
              placeholder="2500"
              className={inputCls}
            />
          </div>
          <div>
            ...
            <select
              ...
              className={inputCls}
            >
              {["prospect", "confirme", "en_cours", "livre", "termine"].map((s) => (
                <option key={s} value={s}>{
                  s === "prospect" ? "Prospect" :
                  s === "confirme" ? "Confirmé" :
                  s === "en_cours" ? "En cours" :
                  s === "livre" ? "Livré" : "Terminé"
                }</option>
              ))}
            </select>
          </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          />
          <label htmlFor="acompte_verse" className="text-sm text-[#261E14]/60 cursor-pointer">
            Acompte déjà versé
          </label>
        </div>
      </div>
      ...
      <div className="glass border border-[#E8A87C]/15 p-6 rounded-sm">
        ...
        <textarea
          ...
          rows={4}
          placeholder="Remarques, préférences, besoins particuliers…"
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        >
          {saving ? (
            <span className="w-3.5 h-3.5 border-2 border-[#261E14]/30 border-t-[#261E14] rounded-full animate-spin" />
          ) : (
            <Save size={13} />
          )}
          Créer le client
        </button>
        <Link href="/admin/clients" className="text-xs text-[#261E14]/30 hover:text-[#261E14]/60 transition-colors">
          Annuler
        </Link>
      </div>
    </form>
