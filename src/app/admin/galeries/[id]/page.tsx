...
import { PublierGalerieButton } from "./GalerieActions";
    ...
    <AdminShell>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <p className="text-xs tracking-[0.4em] uppercase text-[#C4A5B5]/50 mb-1">Galerie</p>
      ...
      <div className="glass border border-[#C4A5B5]/15 overflow-hidden">
        <div className="px-5 py-4 border-b border-[#C4A5B5]/10 flex items-center justify-between">
          <span className="text-xs tracking-widest uppercase text-[#1A1520]/40">Fichiers ({fichiers?.length ?? 0})</span>
        </div>
        <div className="divide-y divide-[#C4A5B5]/8">
          {fichiers && fichiers.length > 0 ? fichiers.map((f: any) => (
            <div key={f.id} className="flex items-center gap-4 px-5 py-3 hover:bg-[#C4A5B5]/3 transition-colors">
              <div className="text-[#C4A5B5]/40">
                {f.type === "photo" ? <Image size={14} /> : f.type === "video" ? <Video size={14} /> : <FileArchive size={14} />}
              </div>
              <div className="flex-1 text-sm text-[#1A1520]/70 truncate">{f.nom}</div>
              <div className="text-xs text-[#1A1520]/30">{f.type}</div>
              <a
                href={f.url}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs text-[#C4A5B5]/40 hover:text-[#C4A5B5] transition-colors"
              >
                Voir →
              </a>
            </div>
