...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { background: #0A0A0A; font-family: Georgia, serif; color: #FAFAFA; }
    .wrapper { max-width: 600px; margin: 0 auto; }
    .header { padding: 40px 40px 0; text-align: center; border-bottom: 1px solid rgba(201,168,76,0.2); padding-bottom: 32px; }
    .logo { font-size: 28px; letter-spacing: 0.15em; color: #C9A84C; font-weight: bold; text-transform: uppercase; }
    .logo-sub { font-size: 10px; letter-spacing: 0.4em; color: rgba(201,168,76,0.5); text-transform: uppercase; margin-top: 4px; }
    .content { padding: 40px; }
    h1 { font-size: 24px; color: #FAFAFA; margin-bottom: 16px; line-height: 1.3; }
    p { font-size: 15px; color: rgba(250,250,250,0.6); line-height: 1.7; margin-bottom: 16px; }
    .highlight { color: #C9A84C; }
    .box { background: rgba(201,168,76,0.05); border: 1px solid rgba(201,168,76,0.2); padding: 24px; margin: 24px 0; }
    ...
    .btn { display: inline-block; background: #C9A84C; color: #0A0A0A; padding: 14px 32px; font-size: 12px; letter-spacing: 0.2em; text-transform: uppercase; font-weight: bold; text-decoration: none; margin: 24px 0; }
    .footer { padding: 24px 40px; border-top: 1px solid rgba(201,168,76,0.1); text-align: center; }
...
<body>
  <div class="wrapper">
    ...
    <div class="footer">
      ...
      <p style="margin-top:6px"><a href="https://www.irzzenproductions.fr" style="color:rgba(201,168,76,0.4);text-decoration:none;">www.irzzenproductions.fr</a></p>
    </div>
  </div>
</body>
</html>`;
...
function emailConfirmationContact({ nomClient, dateMariage, formule, total }: {
  ...
  return baseLayout(`
    ...
    <p>Nous avons bien reçu votre demande et nous vous répondrons dans les <strong style="color:#FAFAFA">24 heures</strong> avec nos disponibilités et votre devis personnalisé.</p>
    ${formule ? `
    <div class="box">
      <div style="font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:rgba(201,168,76,0.6);margin-bottom:16px;">Votre sélection</div>
      ...
      ${total ? `<div class="box-row"><span class="box-label">Total estimé</span><span class="box-value" style="color:#C9A84C">${total.toLocaleString("fr-FR")} €</span></div>` : ""}
    </div>` : ""}
...
function emailNotificationAdmin({ nomClient, email, telephone, dateMariage, lieu, formule, total, message }: {
  ...
  return baseLayout(`
    ...
    <div class="box">
      <div style="font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:rgba(201,168,76,0.6);margin-bottom:16px;">Informations client</div>
    ...
    ${message ? `<div class="box"><div style="font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:rgba(201,168,76,0.6);margin-bottom:12px;">Message</div><p style="margin:0;color:rgba(250,250,250,0.7);font-size:14px;">${message}</p></div>` : ""}
...
function emailGalerieDisponible({ nomClient, nomGalerie, nbPhotos }: {
  ...
  return baseLayout(`
    ...
    <p>Nous avons le plaisir de vous annoncer que votre galerie <strong style="color:#FAFAFA">${nomGalerie}</strong> est maintenant disponible dans votre espace client.</p>
    <div class="box">
      <div style="font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:rgba(201,168,76,0.6);margin-bottom:16px;">Votre galerie</div>
      <div class="box-row"><span class="box-label">Galerie</span><span class="box-value">${nomGalerie}</span></div>
      <div class="box-row"><span class="box-label">Nombre de photos</span><span class="box-value" style="color:#C9A84C">${nbPhotos} photos</span></div>
    </div>
...
function emailContratASigner({ nomClient, titreContrat }: {
  ...
  return baseLayout(`
    ...
    <p>Votre contrat de prestation <strong style="color:#FAFAFA">${titreContrat}</strong> est disponible dans votre espace client et attend votre signature électronique.</p>
    <div class="box">
