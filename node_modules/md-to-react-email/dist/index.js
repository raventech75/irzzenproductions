...
`;
    ...
    heading(text, level) {
      return `<h${level}${parseCssInJsToInlineCss(
        ...
        finalStyles[`h${level}`]
      )}"` : ""}${withDataAttr ? ' data-id="react-email-heading"' : ""}>${text}</h${level}>`;
    },
    hr() {
      return `<hr${withDataAttr ? ' data-id="react-email-hr"' : ""}${parseCssInJsToInlineCss(finalStyles.hr) !== "" ? ` style="${parseCssInJsToInlineCss(finalStyles.hr)}"` : ""} />
`;
    },
    image(href, _, text) {
    ...
    link(href, _, text) {
      let out = `<a href="${href}" target="_blank"${withDataAttr ? ' data-id="react-email-link"' : ""}${parseCssInJsToInlineCss(finalStyles.link) !== "" ? ` style="${parseCssInJsToInlineCss(finalStyles.link)}"` : ""}>${text}</a>`;
      return out;
    },
...
`;
    ...
    paragraph(text) {
      return `<p${parseCssInJsToInlineCss(finalStyles.p) !== "" ? ` style="${parseCssInJsToInlineCss(finalStyles.p)}"` : ""}${withDataAttr ? ' data-id="react-email-text"' : ""}>${text}</p>
`;
    },
    strong(text) {
...
// src/parseMarkdownToReactEmailJSX.ts
var parseMarkdownToReactEmailJSX = ({
  markdown,
  customStyles,
  withDataAttr
}) => {
...
exports.ReactEmailMarkdown = ReactEmailMarkdown; exports.camelToKebabCase = camelToKebabCase; exports.parseCssInJsToInlineCss = parseCssInJsToInlineCss; exports.parseMarkdownToReactEmailJSX = parseMarkdownToReactEmailJSX;
